...
public class StringsComparisonMutant
{
	/// <summary>
	...
	private static int EditDistance(string str1, string str2)
	{
		...
		for (var i = 1; i <= length1; ++i)
		{
			...
			{
                // Mutation 3: change matching logic
				var num = str2[j - 1] == str1[i - 1] ? 1 : 0;
				var numArray2 = new[]
				{
					numArray[i - 1, j] + 1,
					numArray[i, j - 1] + 1,
	...
	public static double CompareStrings(string str1, string str2)
	{
		...
		while (true)
		{
            // Mutation 2: change double max to min
			var deleteElem = originalArray.Cast<double>().Min();
